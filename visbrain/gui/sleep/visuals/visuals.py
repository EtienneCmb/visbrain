...
class ChannelPlot(PrepareData):
    ...
    def __init__(self, channels, time, color=(.2, .2, .2), width=1.5,
        ...
        for i, k in enumerate(channels):
            ...
            # ----------------------------------------------
            # Create threshold line (for channel plot) :
            if k == 'FpZ':
                thresline_top = scene.InfiniteLine(37.5, vertical = False, 
                                                   color = [0.5, 0.5, 1.0, 0.5], parent = node)
                thresline_bottom = scene.InfiniteLine(-37.5, vertical = False, 
                                                      color = [0.5, 0.5, 1.0, 0.5], parent = node)
